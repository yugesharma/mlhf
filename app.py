...
RESPONSE_LENGTH      = Summary('app_response_length_tokens',        'Length of responses in tokens')
TEAM_COUNT = Counter('app_team_in_responses', 'Number of times a team is mentioned in responses', ['team'])
...
def instrument(func):
    def wrapper(hf_token: gr.OAuthToken, *args, **kwargs):
        ...
        try:
            ...
            RESPONSE_LENGTH.observe(len(result.split()))
            return result
...
def chatBot(
    ...
    if 'team' in metadata[index[0][0]]:
        TEAM_COUNT.labels(team=metadata[index[0][0]]['team']).inc()
    else:
        TEAM_COUNT.labels(team="unknown").inc()
